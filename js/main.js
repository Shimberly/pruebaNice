...
function recibir(){
    ...
    $.getJSON('info.json', function(data){
        ...
        if(i==recoger){
            ...
            $.each(lista, function (i, objPieza) {
                    $(".piezas").append("<div class='col-md-4'>"+objPieza.orden+"<img id='s"+objPieza.orden+"' draggable='true' ondragstart='drag(event)' class='imgPieza' src='"+objPieza.imagen+"'></div>");

                    $(".fondoRmp").append("<div id='ss"+(i+1)+"' ondrop='drop(event)' ondragover='allowDrop(event)' class='col-md-4 fondoPz'>"+(i+1)+"</div>");
                });
